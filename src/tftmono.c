...
/* -------------------------------------------------------
  ...
  // Anpassung an bestehende Programme
  #define i2c_stop()     (i2c_send_stop(I2C2))
  ...
  /*  ---------------------------------------------------------
        ...
        initialisiert I2C_2 Schnittstelle (GPIOB_10 und
        GPIOB_11) im Standardmodus (100 kHz) und setzt die
        Alternativfunktion fuer diese Pins
      --------------------------------------------------------- */
  void i2c_init(void)
  {
    // Takt einschalten fuer I2C_2 und die entsprechenden GPIO
    rcc_periph_clock_enable(RCC_I2C2);
    rcc_periph_clock_enable(RCC_AFIO);

    // Alternativfunktion fuer GPIOB_10 (scl) und GPIOB_11 (sda)
    gpio_set_mode(GPIOB, GPIO_MODE_OUTPUT_50_MHZ, GPIO_CNF_OUTPUT_ALTFN_OPENDRAIN, GPIO10 | GPIO11);

    i2c_peripheral_disable(I2C2);                                   // I2C vor Aenderungen ausschalten
    i2c_set_clock_frequency(I2C2, I2C_CR2_FREQ_24MHZ);              // I2CC Peripherie wird mit 24 MHz getaktet
//    i2c_set_fast_mode(I2C2);                                        // I2C fast-mode (400KHz)
    i2c_set_standard_mode(I2C2);                                    // I2C standard-mode (100KHz)

    i2c_set_ccr(I2C2, 0x1e);                                        // Datenblattempfehlung fuer 24..36MHz
    i2c_set_trise(I2C2, 0x0b);                                      // dto.

    // i2c_set_own_7bit_slave_address(I2C2, 0x32);                  // nur bei anderem Master als dem F103 benoetigt

    i2c_peripheral_enable(I2C2);                                    // nach Konfiguration, I2C einschalten
  }
  ...
  /*  ---------------------------------------------------------
        ...
        Uebergabe:
           addr : 8 Bitadresse des anzusprechenden Slaves
           rw   : Read/Write Flag: 0 => write; 1 => read
      --------------------------------------------------------- */
  void i2c_start(uint8_t addr, uint8_t rw)
  {
    addr = addr >> 1;                     // I2C Adresse als 8-Bit Adresse, LSB = r/w Bit in Adresse enthalten
    i2c_send_start(I2C2);

    // warten bis Start-Kondiiton gesendet und in den Master-Mode geschaltet ist
    while (!((I2C_SR1(I2C2) & I2C_SR1_SB) & (I2C_SR2(I2C2) & (I2C_SR2_MSL | I2C_SR2_BUSY))));

    i2c_send_7bit_address(I2C2, addr, rw);                        // Zieladresse senden (0x78 >> 1 = 0x3c fuer SSD1306 Controller)
    while (!(I2C_SR1(I2C2) & I2C_SR1_ADDR));                      // warten, bis Transfer beendet ist

    reg32 = I2C_SR2(I2C2);                                        // ADDR Sequenz loeschen
  }
  ...
  void i2c_write(uint8_t value)
  {
    ...
    i2c_send_data(I2C2, value);                                   // Wert senden
...
/*  ---------------------------------------------------------
  ...
  void lcd_init(void)
  {
    ...
    i2clcd_writecmd(0x8d);      // Ladungspumpe an
    i2clcd_writecmd(0x14);
    i2clcd_writecmd(0xaf);      // Display an
    delay(150);
    i2clcd_writecmd(0xa1);      // Segment Map
    i2clcd_writecmd(0xc0);      // Direction Map
