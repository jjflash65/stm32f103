/* ----------------------------------------------------------
     ...
     Der Controller wird mit 64 Mhz und interner oder 72 MHz
     und externer Taktquelle initialisiert.
...
//  systick_set_clocksource(STK_CSR_CLKSOURCE_EXT);
  ...
  #if (coreclock_intern)
    systick_set_reload(8000);                      // 64 MHz / 8 = 8 MHz => (1 / 8 MHz) * 8000 = 1 ms
  #else
    systick_set_reload(9000);                      // 72 MHz / 8 = 9 MHz => (1 / 9 MHz) * 9000 = 1 ms
  #endif
...
/* -------------------------------------------------------
   ...
   64 MHz mit interner oder 72 MHz mit externer
   Taktquelle
 ------------------------------------------------------- */
void sys_init(void)
{
